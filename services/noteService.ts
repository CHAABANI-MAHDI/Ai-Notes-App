...
export const noteService = {
  ...
  // Get the total count of notes for a specific user (used for plan limits)
  getCountForUser: async (userId: string): Promise<number> => {
    const { count, error } = await supabase
      .from('notes')
      .select('id', { count: 'exact', head: true })
      .eq('user_id', userId);
  ...
  create: async (note: Omit<Note, 'id' | 'createdAt' | 'updatedAt'>): Promise<Note> => {
    ...
    const currentCount = await noteService.getCountForUser(user.id);
    if (currentCount >= 5) {
